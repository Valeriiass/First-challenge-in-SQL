...
SELECT COUNT(DISTINCT state) AS distinct_states FROM authors;

-- 2. How many publishers are based in the USA?

SELECT COUNT(country) FROM publishers WHERE country='USA';

-- 3. What's the average quantity of titles sold per sale by store 7131?

SELECT AVG(qty) AS avg_qty_st7131 FROM sales WHERE stor_id=7131;
...
SELECT AVG(price)AS avg_price_psychology FROM titles WHERE type='psychology';
...
-- business books more then popular comp

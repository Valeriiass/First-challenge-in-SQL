...
SELECT * FROM titles WHERE title LIKE '%cooking%';
...
SELECT emp_id FROM employee WHERE emp_id LIKE ('%F');

/* 7. Select the first and last names of all male employees whose name starts
   with "P". */

SELECT lname, fname FROM employee WHERE emp_id LIKE ('%M') AND fname LIKE ('P%');
